...
async def award_badge(client: Client, message: Message):
    ...
    await message.reply_animation(animation="https://files.catbox.moe/rtgatv.mp4", caption=f"• Name: {replied_user.first_name}\n• ID: {replied_user.id}\n\n**Has been awarded the {leader['region']} {badge}.**")
...
async def accept_challenge(client, callback_query):
    ...
    if match:
        user_id_str, group_id_str = match.groups()
        ...
        await callback_query.message.edit_text("The challenge has been accepted!")
...
async def decline_challenge(client, callback_query):
    ...
    if match:
        ...
        await callback_query.message.edit_text("The challenge has been declined.")
        await client.send_message(chat_id=user_id, text="Your challenge has been declined.")
